defmodule AcqdatIotWeb.DataDump do
  ...
  describe "create/2" do
    ...
    test "data dump create", %{conn: conn, org: org, gateway: gateway} do
      ...
      conn = post(conn, Routes.data_dump_path(conn, :create, org.id, project.id, gateway.id), params)
    ...
    test "fails if authorization header not found", %{conn: conn, org: org, gateway: gateway} do
      ...
      data = %{}
      conn = post(conn, Routes.data_dump_path(conn, :create, org.id, gateway.project.id, gateway.id), data)
